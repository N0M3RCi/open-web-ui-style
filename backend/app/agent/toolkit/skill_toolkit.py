...
User isolation is managed via ~/.merci/user_<id>/skills-config.json.
"""
...
def _get_user_config_path(user_id: str | None = None) -> Path:
    ...
    """
    if user_id:
        # User-specific config: ~/.merci/user_<id>/skills-config.json
        user_key = canonical_skill_config_user_id(user_id)
        return Path.home() / ".merci" / user_key / SKILL_CONFIG_FILENAME
    else:
        # Legacy global config: ~/.merci/skills-config.json
        return Path.home() / ".merci" / SKILL_CONFIG_FILENAME
...
def _get_merged_skill_config(
    ...
    project_config_path = wd / ".merci" / SKILL_CONFIG_FILENAME
...
class SkillToolkit(BaseSkillToolkit):
    """Enhanced SkillToolkit with M3RCI - UniMind-specific features.

    Extends CAMEL's SkillToolkit with:
    - User-specific skill configuration
    - Agent-based access control
    - M3RCI - UniMind-specific skill paths (.merci/skills)

    Skill Discovery Priority (highest to lowest):
    1. Repo scope: <wd>/skills, <wd>/.merci/skills, <wd>/.camel/skills
    2. User scope: ~/.merci/skills, ~/.camel/skills, ~/.config/camel/skills
    ...
    def __init__(
        ...
        """Initialize SkillToolkit with M3RCI - UniMind-specific context.
    ...
    def _skill_roots(self) -> list[tuple[str, Path]]:
        ...
        Integrates M3RCI - UniMind-specific paths with CAMEL standard paths.

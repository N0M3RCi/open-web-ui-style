...
Add the following environment variables to `~/.nova/.env`:
...
{
  "resourceAttributes": {
    "service.name": "nova-workforce",
    "nova.project.id": "1768815931733-6575",
    "nova.task.id": "1768815944094-9806"
  },
  "scope": {
    "name": "nova.workforce",
    "version": "0.2.83a9"
...
{
  "attributes": {
    "nova.worker.id": "73d20286-2c17-467a-8153-2a6ea8cbb6c2",
    "nova.worker.type": "SingleAgentWorker",
    "nova.worker.role": "Developer Agent: A master-level coding...",
    "nova.worker.agent": "developer_agent",
    "nova.worker.model.type": "gpt-4.1-mini"
  }
}
```
...
{
  "attributes": {
    "nova.task.id": "1768815944094-9806.1",
    "nova.task.description": "Task description",
    "nova.project.id": "1768815931733-6575",
    "nova.task.parent_id": "1768815944094-9806",
    "nova.task.type": "task_type"
  }
}
```
...
```json
{
  "attributes": {
    "nova.task.id": "1768815944094-9806.1",
    "nova.worker.id": "0fae2d3d-7c0a-4b50-b09d-da35ae61786d",
    "nova.project.id": "1768815931733-6575",
    "nova.task.queue_time_seconds": "1.5",
    "nova.task.dependencies": "[\"dep_1\", \"dep_2\"]"
  }
}
```
...
```json
{
  "attributes": {
    "nova.task.id": "1768815944094-9806.1",
    "nova.project.id": "1768815931733-6575",
    "nova.task.status": "completed",
    "nova.worker.id": "0fae2d3d-7c0a-4b50-b09d-da35ae61786d",
    "nova.task.timestamp": "2026-01-19T09:46:40.045077+00:00",
    "nova.task.parent_id": "1768815944094-9806",
    "nova.task.processing_time_seconds": "10.926168203353882",
    "nova.task.quality_score": "80",
    "nova.task.token_usage.total_tokens": "37284"
  }
}
```
...
```json
{
  "attributes": {
    "nova.project.id": "1768815931733-6575",
    "nova.task.id": "1768815944094-9806",
    "nova.task.timestamp": "2026-01-19T09:46:44.901068+00:00",
...
- `nova.project.id` - Workforce/project identifier
- `nova.task.id` - Task identifier
- `nova.task.description` - Task description
- `nova.task.parent_id` - Parent task ID
- `nova.task.type` - Task type
- `nova.task.status` - Task status (started, completed, failed)
- `nova.task.timestamp` - ISO 8601 timestamp
- `nova.task.dependencies` - JSON array of dependency task IDs
- `nova.task.queue_time_seconds` - Time in queue before assignment
- `nova.task.processing_time_seconds` - Task execution duration
- `nova.task.quality_score` - Quality score (0-100)

### Worker

- `nova.worker.id` - Worker UUID
- `nova.worker.type` - Worker class type
- `nova.worker.role` - Worker role description
- `nova.worker.agent` - Agent type (developer_agent, browser_agent, etc.)
- `nova.worker.model.type` - Model name (gpt-4, claude-3, etc.)

### Token Usage

- `nova.task.token_usage.total_tokens` - Total tokens used
- `nova.task.token_usage.*` - Additional token usage metrics

### Langfuse

- `langfuse.session.id` - Set to project ID for grouping
- `langfuse.tags` - ["workforce", "camel", "nova"]

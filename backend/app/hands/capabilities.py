...
class BrainCapabilities:
    ...
    workspace_root: Path = field(
        default_factory=lambda: Path("~/.merci/workspace").expanduser()
    )
    """workspace root path"""
...
def detect_capabilities(config: dict | None = None) -> BrainCapabilities:
    ...
    1. Deployment env: MERCI_DEPLOYMENT_TYPE / Docker auto-detect
    2. Env var overrides: MERCI_HANDS_*
    """
    cfg = config or {}

    # 1. Deployment env determines base capabilities
    deployment = env("MERCI_DEPLOYMENT_TYPE") or ""
    ...
    if deployment in DEPLOYMENT_FULL:
        ...
        if in_docker:
            ...
            caps = BrainCapabilities(
                ...
                workspace_root=Path(
                    env("MERCI_WORKSPACE", "~/.merci/workspace")
                ).expanduser(),
    ...
    # 2. Env var overrides
    if env("MERCI_HANDS_TERMINAL") is not None:
        caps.has_terminal = env("MERCI_HANDS_TERMINAL", "true").lower() in (
            "1",
            "true",
            "yes",
        )
    if env("MERCI_HANDS_BROWSER") is not None:
        caps.has_browser = env("MERCI_HANDS_BROWSER", "false").lower() in (
            "1",
            "true",
            "yes",
        )
    if env("MERCI_HANDS_FILESYSTEM") is not None:
        caps.filesystem_scope = env("MERCI_HANDS_FILESYSTEM", "full")
    if env("MERCI_HANDS_MCP") is not None:
        caps.mcp_mode = env("MERCI_HANDS_MCP", "all")
    if env("MERCI_CDP_URL"):

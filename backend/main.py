...
_fallback_camel_log_dir = (
    pathlib.Path.home() / ".merci" / "fallback" / "camel_logs"
)
...
app_logger.info("Starting M3RCI - UniMind Multi-Agent System API")
...
def run_standalone():
    ...
    port = int(env("MERCI_BRAIN_PORT", "5001"))
    host = env("MERCI_BRAIN_HOST", "0.0.0.0")  # nosec B104 - bind all for Docker/dev
    reload = os.environ.get("MERCI_DEBUG", "").lower() in ("1", "true", "yes")

...
class TestChatController:
    ...
    async def test_post_chat_sets_cdp_url_when_browser_ready(
        ...
        """Web mode should set MERCI_CDP_URL after successful browser ensure."""
        chat_data = Chat(**sample_chat_data)
    ...
    async def test_post_chat_clears_cdp_url_when_browser_unavailable(
        ...
        """Web mode should mark browser unavailable and clear MERCI_CDP_URL."""
        chat_data = Chat(**sample_chat_data)
        mock_request.state = SimpleNamespace()
        ...
        with (
            ...
            patch.dict(
                os.environ,
                {"MERCI_CDP_URL": "http://127.0.0.1:9222"},
                clear=True,
            ),
        ):
    ...
    async def test_post_chat_preserves_existing_cdp_url(
        ...
        with (
            ...
            patch.dict(
                os.environ,
                {"MERCI_CDP_URL": "http://worker-17:9222"},
                clear=True,
            ),
        ):

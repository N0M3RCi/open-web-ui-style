...
function injectSandboxStorageShim(html: string): string {
  if (!html || html.includes('data-merci-sandbox-storage-shim')) {
    return html;
  }

  const shim = `<script data-merci-sandbox-storage-shim>
(function () {
...
export function FileViewerPanel({
  ...
  return (
    <div
      className={`mb-sm flex min-w-0 flex-1 flex-col overflow-hidden rounded-xl ${surfaceClassName}`}
    >
      {/* head */}
      {selectedFile && (

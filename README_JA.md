...
### Nova: 卓越した生産性を実現するオープンソースのコワークデスクトップ
...
**Nova**は、オープンソースのコワークデスクトップアプリケーションです。複雑なワークフローを自動化タスクに変換できるカスタムAIワークフォースを構築、管理、デプロイする力を提供します。先進的なオープンソース Cowork製品として、NovaはオープンソースコラボレーションとAI駆動の自動化の最良の部分を組み合わせています。
...
> **🔓 オープンに開発** — Novaは初日から**100%オープンソース**です。すべての機能、すべてのコミット、すべての決定が透明です。最高のAIツールは、閉じられたドアの後ろではなく、コミュニティと共にオープンに構築されるべきだと信じています。

### 🏠 ローカルデプロイメント（推奨）

Novaを実行する推奨方法 — データを完全に制御でき、クラウドアカウント不要で完全にスタンドアロンで動作します。
...
> 注：このモードはNovaクラウドサービスに接続し、アカウント登録が必要です。完全にスタンドアロンで使用する場合は、代わりに[ローカルデプロイメント](#-%E3%83%AD%E3%83%BC%E3%82%AB%E3%83%AB%E3%83%87%E3%83%97%E3%83%AD%E3%82%A4%E3%83%A1%E3%83%B3%E3%83%88%E6%8E%A8%E5%A5%A8)を使用してください。
...
📧 詳細については、[info@nova.ai](mailto:info@nova.ai) までお問い合わせください。

### ☁️ クラウドバージョン

マネージドインフラストラクチャを好むチーム向けに、クラウドプラットフォームも提供しています。セットアップの複雑さなしにNovaのマルチエージェントAI機能を体験する最速の方法です。モデル、API、クラウドストレージをホストし、Novaがシームレスに動作することを保証します。
...
<div align="right">
<a href="https://www.merci-unimind.ai/download">Nova.aiで始める →</a>
</div>

## **✨ 主な機能 - オープンソース Cowork**

Novaのオープンソース Coworkの強力な機能で卓越した生産性の可能性を最大限に引き出しましょう — シームレスな統合、よりスマートなタスク実行、無限の自動化のために構築されています。

### 🏭 ワークフォース

複雑なタスクを解決するために協力する専門AIエージェントのチームを活用します。Novaのオープンソース Coworkは動的にタスクを分解し、複数のエージェントを**並列で**動作させます。

Novaは以下のエージェントワーカーを事前定義しています：
...
お好みのモデルでNova オープンソース Coworkデスクトップをローカルにデプロイできます。
...
Novaには大規模な組み込み\*\*Model Context Protocol（MCP）\*\*ツール（ウェブブラウジング、コード実行、Notion、Google suite、Slackなど）が付属しており、**独自のツールをインストール**することもできます。エージェントにシナリオに適したツールを装備させ、内部APIやカスタム関数を統合して機能を強化できます。
...
タスクが行き詰まったり不確実性に遭遇した場合、Novaは自動的に人間の入力を要求します。
...
Novaは完全にオープンソースです。コードをダウンロード、検査、修正でき、透明性を確保し、マルチエージェントイノベーションのためのコミュニティ主導のエコシステムを育成します。
...
世界中の開発者がNovaのオープンソース Cowork機能を活用して、さまざまな業界で複雑なワークフローを自動化し、生産性を向上させている方法をご覧ください。
...
Nova オープンソース Coworkデスクトップは、スケーラビリティ、パフォーマンス、拡張性を確保する最新の信頼性の高いテクノロジーで構築されています。
...
> [!IMPORTANT]
>
> **Novaにスター**を付けると、GitHubからすべてのリリース通知を遅延なく受け取れます ⭐️
...
私たちは信頼を築き、あらゆる形式のオープンソースコラボレーションを歓迎することを信じています。あなたの創造的な貢献が`Nova`のイノベーションを推進します。GitHubのissuesとプロジェクトを探索して、あなたの力を見せてください 🤝❤️ [コントリビューションガイドライン][contribution-link]
...
Novaは[CAMEL-AI.org][camel-ai-org-github]の研究とインフラストラクチャの上に構築されています。[CAMEL-AI.orgをスポンサー][sponsor-link]することで`Nova`がより良くなります。
...
詳細については info@nova.ai までお問い合わせください

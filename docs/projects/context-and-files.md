...
- Files stored in an Nova scratch workspace
...
Nova supports previews for common text, source code, Markdown, image, HTML, document, and data formats. Unsupported formats remain available in the workspace even when they cannot be previewed.
...
Blank Spaces use artifact-only mode. Nova stores generated outputs in managed project storage instead of modifying an existing user folder.
...
Nova opens Context and selects the file belonging to that Run.
...
Confirm that the file still exists and that Nova has permission to access the containing folder.
